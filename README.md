...
| Phase                                          | What it does                                                                                                                                                                                                                                                  |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **I. Massive One-Time Data Acquisition** | Highly concurrent scraper for large directories. Targets:**1,000+ startups**, **1,000+ products**, **1,000+ research papers** correlated with GitHub repos and live star counts. Papers come from the arXiv API with repos linked by a three-layer correlator - author-declared links in abstracts, the Hugging Face Papers API, and GitHub citation search (repos citing the paper's arXiv ID) - every link labeled with its provenance (Papers with Code, the suggested source, was sunset in 2025 and now redirects to Hugging Face - the pipeline adapts while keeping every record traceable to a real arXiv URL).                                 |
...
### Paper → GitHub Correlation

Three linkage layers, cheapest and most reliable first, each recorded on the record as `github_source`:

1. **Author-declared** — a GitHub URL in the paper's abstract/comment (deterministic regex).
2. **Hugging Face Papers API** — community-linked repos for the arXiv ID.
3. **GitHub citation search** — repositories whose name/description/README cite the paper's arXiv ID (the technique Papers with Code used to bootstrap its links), guarded against false positives: paper-collection repos (`awesome-*`, `*-papers`) and >50k-star citing frameworks are rejected.

Star counts are always fetched **live from the GitHub API**, never copied from stale sources — and a paper without public code keeps `github_url: null`, never a guess.
